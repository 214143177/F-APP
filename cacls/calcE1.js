function calcE1() {
    ...
    const form = document.getElementById('formE1');
    ...
    if (Asi < 0) {
        alert(`受压区配筋面积为"${Asi.toFixed(0)} mm²"，请优化截面尺寸！`);
        
        return;
    }
    const As = (fyi * Asi + α1 * fc * b * h0 * ξb) / fy; // 计算受拉区配筋面积

    // const x = (fy * As - fyi * Asi) / (α1 * fc * b); // 计算配筋率

    const ρmin = Math.max(0.2, 45 * ft / fy); // 最小配筋率,双筋矩形不考虑，单筋矩形考虑。
    const ρmax = α1 * ξb * fc / fy * 100;// 最大配筋率,双筋矩形不考虑，单筋矩形考虑。

    console.log({ "M": M, "b": b, "h": h, "D": D, "d": d, "c": c, "as": as, "h0": h0, "asi": asi, "εcu": εcu, "ξb": ξb, "ξ": ξ, "Asi": Asi, "As": As, "ρmin": ρmin, "ρmax": ρmax })
    // 结果输出
